<x-app-layout>
    <x-slot name="header"><h2 class="text-xl font-semibold">Add DSA</h2></x-slot>
    <form method="POST" action="{{ route('dsas.store') }}" class="p-6 bg-white rounded shadow max-w-2xl mx-auto">
        @csrf
        <div class="grid grid-cols-2 gap-4">
            <div><label>Name</label><input name="name" class="w-full border rounded p-2" required></div>
            <div><label>Email</label><input name="email" type="email" class="w-full border rounded p-2" required></div>
            <div><label>Phone</label><input name="phone" class="w-full border rounded p-2" required></div>
            <div><label>Company</label><input name="company_name" class="w-full border rounded p-2"></div>
